...
import { Snippet, UpdateSnippetRequest } from '@/lib/types';

// Edit snippet form schema
...
type EditSnippetFormData = z.infer<typeof editSnippetSchema>;
...
const EditSnippet: React.FC = () => {
  ...
  const form = useForm<EditSnippetFormData>({
  ...
  useEffect(() => {
    const fetchSnippet = async () => {
      ...
      try {
        ...
        // Set tags
        setTags(snippetData.tags.map(tag => tag.name));
  ...
  const onSubmit = async (data: EditSnippetFormData) => {
    if (!id) return;
    ...
    try {
      ...
      const snippetData: UpdateSnippetRequest = {
        title: data.title,
      ...
      await snippetApi.updateSnippet(parseInt(id), snippetData);
      navigate('/dashboard');
    } catch (error: any) {
      console.error('Update snippet error:', error);
      
      let errorMessage = 'Failed to update snippet. Please try again.';
      ...
      if (error.response) {
        ...
        errorMessage = error.message || 'Failed to update snippet. Please try again.';
      }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Edit Snippet</h1>
          <p className="text-gray-600">Update your code snippet</p>
        </div>
        ...
        <Card className="backdrop-blur-sm bg-white/80 border-0 shadow-2xl">
          <CardHeader className="pb-6">
            ...
            <CardDescription className="text-gray-600">
              Update the details below to modify your code snippet
            </CardDescription>
          </CardHeader>
          <CardContent>
            <Form {...form}>
